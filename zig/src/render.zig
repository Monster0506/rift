...
pub fn render(
    ...
    pending_key: ?key.Key,
    debug_mode: bool,
    debug_info: ?@import("editor.zig").KeyDebugInfo,
) !void {
    // Clear screen
    ...
    try renderStatusBar(term, viewport, current_mode, pending_key, debug_mode, debug_info);
...
fn renderStatusBar(
    ...
    // Debug info (vk, ascii, unicode)
    if (debug_mode) {
        if (debug_info) |info| {
            var debug_buf: [64]u8 = undefined;
            const debug_str = std.fmt.bufPrint(&debug_buf, " vk:0x{X:0>2} ascii:{d} unicode:{d}", .{ info.vk, info.ascii, info.unicode }) catch " vk:?? ascii:?? unicode:??";
            try term.write(debug_str);
        }
    }
    ...
    if (debug_mode and debug_info != null) {
        used_cols += 30; // Approximate length of debug info string
    }
