...
fn test_lines_basic() {
    ...
    assert_eq!(pt.line_at_byte(0), 0);
    assert_eq!(pt.line_at_byte(6), 0); // '1'
    assert_eq!(pt.line_at_byte(7), 1); // 'L' of Line 2
    assert_eq!(pt.line_at_byte(13), 1);
    assert_eq!(pt.line_at_byte(14), 2);
}
...
fn test_byte_access() {
    let mut pt = PieceTable::new(Vec::new());
    pt.insert(0, b"0123456789");

    assert_eq!(pt.byte_at(0), b'0');
    assert_eq!(pt.byte_at(5), b'5');
    assert_eq!(pt.byte_at(9), b'9');

    let range = pt.bytes_range(3..7);
...
fn test_complex_edits() {
    ...
    pt.insert(0, b"The quick brown fox");

    // 2. Insert " jumps over the lazy dog"
    pt.insert(19, b" jumps over the lazy dog");
    assert_eq!(
        pt.to_string(),
...
fn test_original_buffer_usage() {
    let original = b"Original Text".to_vec();
...
#[test]
fn test_line_at_byte_edge_cases() {
    let mut pt = PieceTable::new(Vec::new());
    pt.insert(0, b"\n\n");
    assert_eq!(pt.get_line_count(), 3);

    assert_eq!(pt.line_at_byte(0), 0); // First newline
    assert_eq!(pt.line_at_byte(1), 1); // Second newline
    assert_eq!(pt.line_at_byte(2), 2); // End of buffer (empty line 3)
}
